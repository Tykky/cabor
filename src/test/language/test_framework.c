...
int run_cabor_tests(cabor_test_framework_context* ctx)
{
	...
	CABOR_LOG_TEST("--------------------------------------------------------------------------------------------------");
	CABOR_LOG_TEST_F("  %-60s | %-10s | %-8s | %-6s |", "Test description", "Status", "Time(ns)", "Count");
	CABOR_LOG_TEST("--------------------------------------------------------------------------------------------------");
	for (size_t i = 0; i < ctx->current_size; i++)
	{
		...
		double diff_ns = (end - start) * 1000000000.f;
		if (!result)
		{
			passed++;
            CABOR_LOG_TEST_F("  %-60s | %-10s | %-8.0f | %.3d/%.3d |", test->name, !result ? "Success" : "Failed", diff_ns, i + 1, ctx->current_size);
		}
		else
		{
			failed++;
            CABOR_LOG_ERR_F(" %-60s | %-10s | %-8.0f | %.3d/%.3d |", test->name, !result ? "Success" : "Failed", diff_ns, i + 1, ctx->current_size);
		}
	}
	CABOR_LOG_TEST("--------------------------------------------------------------------------------------------------");
	if (failed)
